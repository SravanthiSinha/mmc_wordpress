<?php
/**
 * Plugin Name: MMC - ABOUT US PAGE
 * Description: Integration of MMC - ABOUT US PAGE with WordPress
 * Version: 1.0.0
 * Author: Sravanthi Sinha
 * Text Domain: mmc-about_us
 * License: GPL-2.0+
 ...
 * Main plugin class
 * 
 * Handles integration of  MMC - ABOUT US PAGE with WordPress
 */
class MMC_ABOUT_US_App {
    /**
     * Plugin instance
     *
     * @var MMC_ABOUT_US_App
     */
    ...
    private function __construct() {
        ...
        // Register shortcode
        add_shortcode('mmc_about_us', array($this, 'render_react_app'));
    ...
    public function activate() {
        ...
        if (version_compare(PHP_VERSION, '7.0', '<')) {
            ...
            wp_die(
                esc_html__('MMC ABOUT_US requires PHP 7.0 or higher.', 'mmc-about_us'),
                esc_html__('Plugin Activation Error', 'mmc-about_us'),
        ...
        if (!file_exists($this->plugin_dir . 'dist') || !is_dir($this->plugin_dir . 'dist')) {
            error_log('MMC ABOUT_US Plugin: dist directory not found during activation.');
        }
    ...
    public function register_assets() {
        ...
        if (!file_exists($plugin_dir . 'dist') || !is_dir($plugin_dir . 'dist')) {
            error_log('MMC ABOUT_US Plugin: dist directory not found.');
            return;
        }
    ...
    private function register_from_manifest($manifest_file) {
        ...
        if (!$manifest || !is_array($manifest)) {
            error_log('MMC ABOUT_US Plugin: Invalid manifest file');
            return;
        }
        ...
        foreach ($manifest as $file => $details) {
            ...
            $handle = 'mmc-about_us-' . sanitize_title(basename($details['file']));
    ...
    public function maybe_enqueue_assets() {
        ...
        if (is_a($post, 'WP_Post') && has_shortcode($post->post_content, 'mmc_about_us')) {
            // Enqueue all registered styles
            $styles = wp_styles();
            foreach ($styles->registered as $handle => $style) {
                if (strpos($handle, 'mmc-about_us') === 0) {
    ...
    private function get_localized_data() {
        return apply_filters('mmc_about_us_localized_data', array(
            'ajaxUrl' => admin_url('admin-ajax.php'),
            'nonce' => wp_create_nonce('mmc_about_us_nonce'),
            'homeUrl' => home_url(),
            'siteUrl' => site_url(),
    ...
    public function render_react_app($atts) {
        ...
        $atts = shortcode_atts(array(
            'id' => 'mmc-about_us-' . uniqid(), // Generate a unique ID
            'class' => '',
        ...
        return sprintf(
            ...
            '<noscript>' . esc_html__('JavaScript is required to view the ABOUT_US content.', 'mmc-about_us') . '</noscript>' .
            '<p class="react-loading-placeholder">%s</p>' .
            '</div>',
            esc_attr($atts['id']),
            esc_attr($atts['class']),
            $data_attrs,
            esc_html__('Loading ABOUT_US content...', 'mmc-about_us')
        );
    }
    ...
    public function add_debug_script() {
        ...
        <script>
        (function() {
            console.group('MMC ABOUT_US Debug Information');
                ...
                setTimeout(function() {
                    ...
                    if (placeholders.length > 0) {
                        ...
                        const reactScripts = Array.from(scripts).filter(script => 
                            script.src && (script.src.includes('mmc-about_us') || script.src.includes('dist/assets'))
                        );
 ...
 * @return MMC_ABOUT_US_App
 */
function mmc_about_us_plugin() {
    return MMC_ABOUT_US_App::get_instance();
}
