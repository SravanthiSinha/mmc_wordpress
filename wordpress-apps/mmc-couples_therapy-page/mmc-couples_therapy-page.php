<?php
/**
 * Plugin Name: MMC - COUPLES_THERAPY
 * Description: Integration of React Vite app with WordPress
 * Version: 1.0.0
 * Author: Sravanthi Sinha
 * Text Domain: mmc-couples_therapy
 * License: GPL-2.0+
 ...
 */
class MMC_COUPLES_THERAPY_App {
    /**
     * Plugin instance
     *
     * @var MMC_COUPLES_THERAPY_App
     */
     ...
     * Get plugin instance
     *
     * @return MMC_COUPLES_THERAPY_App
     */
    ...
    private function __construct() {
        ...
        // Register shortcode
        add_shortcode('mmc_couples_therapy', array($this, 'render_react_app'));
    ...
    public function activate() {
        ...
        if (version_compare(PHP_VERSION, '7.0', '<')) {
            ...
            wp_die(
                esc_html__('MMC COUPLES_THERAPY requires PHP 7.0 or higher.', 'mmc-couples_therapy'),
                esc_html__('Plugin Activation Error', 'mmc-couples_therapy'),
        ...
        if (!file_exists($this->plugin_dir . 'dist') || !is_dir($this->plugin_dir . 'dist')) {
            error_log('MMC COUPLES_THERAPY Plugin: dist directory not found during activation.');
        }
    ...
    public function register_assets() {
        ...
        if (!file_exists($plugin_dir . 'dist') || !is_dir($plugin_dir . 'dist')) {
            error_log('MMC COUPLES_THERAPY Plugin: dist directory not found.');
            return;
        }
    ...
    private function register_from_manifest($manifest_file) {
        ...
        if (!$manifest || !is_array($manifest)) {
            error_log('MMC COUPLES_THERAPY Plugin: Invalid manifest file');
            return;
        }
        ...
        foreach ($manifest as $file => $details) {
            ...
            $handle = 'mmc-couples_therapy-' . sanitize_title(basename($details['file']));
    ...
    private function register_direct() {
        ...
        foreach ($css_files as $index => $css_file) {
            ...
            $handle = 'mmc-couples_therapy-css-' . $index;
        ...
        foreach ($js_files as $index => $js_file) {
            ...
            $handle = 'mmc-couples_therapy-js-' . $index;
    ...
    public function maybe_enqueue_assets() {
        ...
        if (is_a($post, 'WP_Post') && has_shortcode($post->post_content, 'mmc_couples_therapy')) {
            // Enqueue all registered styles
            $styles = wp_styles();
            foreach ($styles->registered as $handle => $style) {
                if (strpos($handle, 'mmc-couples_therapy') === 0) {
    ...
    private function get_localized_data() {
        return apply_filters('mmc_couples_therapy_localized_data', array(
            'ajaxUrl' => admin_url('admin-ajax.php'),
            'nonce' => wp_create_nonce('mmc_couples_therapy_nonce'),
            'homeUrl' => home_url(),
            'siteUrl' => site_url(),
    ...
    public function render_react_app($atts) {
        ...
        $atts = shortcode_atts(array(
            'id' => 'mmc-couples_therapy-' . uniqid(), // Generate a unique ID
            'class' => '',
            'category' => '', // Optional category filter
        ), $atts, 'mmc_couples_therapy');
        ...
        return sprintf(
            ...
            '<noscript>' . esc_html__('JavaScript is required to view the COUPLES_THERAPY content.', 'mmc-couples_therapy') . '</noscript>' .
            '<p class="react-loading-placeholder">%s</p>' .
            '</div>',
            esc_attr($atts['id']),
            esc_attr($atts['class']),
            $data_attrs,
            esc_html__('Loading COUPLES_THERAPY content...', 'mmc-couples_therapy')
        );
    }
    ...
    public function add_debug_script() {
        ...
        <script>
        (function() {
            console.group('MMC COUPLES_THERAPY Debug Information');
                ...
                setTimeout(function() {
                    ...
                    if (placeholders.length > 0) {
                        ...
                        const reactScripts = Array.from(scripts).filter(script => 
                            script.src && (script.src.includes('mmc-couples_therapy') || script.src.includes('dist/assets'))
                        );
 ...
 * @return MMC_COUPLES_THERAPY_App
 */
function mmc_couples_therapy_plugin() {
    return MMC_COUPLES_THERAPY_App::get_instance();
}

// Initialize the plugin
mmc_couples_therapy_plugin();