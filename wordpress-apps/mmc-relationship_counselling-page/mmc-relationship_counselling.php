<?php
/**
 * Plugin Name: MMC - Relationship Counselling PAGE
 * Description: Integration of MMC - Relationship Counselling PAGE with WordPress
 * Version: 1.0.0
 * Author: Sravanthi Sinha
 * Text Domain: mmc-relationship_counselling
 * License: GPL-2.0+
 ...
 * Main plugin class
 * 
 * Handles integration of  MMC - Relationship Counselling PAGE with WordPress
 */
class MMC_RELATIONSHIP_COUNSELLING_App {
    /**
     * Plugin instance
     *
     * @var MMC_RELATIONSHIP_COUNSELLING_App
     */
     ...
     * Get plugin instance
     *
     * @return MMC_RELATIONSHIP_COUNSELLING_App
     */
    ...
    private function __construct() {
        ...
        // Register shortcode
        add_shortcode('mmc_relationship_counselling', array($this, 'render_react_app'));
    ...
    public function activate() {
        ...
        if (version_compare(PHP_VERSION, '7.0', '<')) {
            ...
            wp_die(
                esc_html__('MMC RELATIONSHIP_COUNSELLING requires PHP 7.0 or higher.', 'mmc-relationship_counselling'),
                esc_html__('Plugin Activation Error', 'mmc-relationship_counselling'),
        ...
        if (!file_exists($this->plugin_dir . 'dist') || !is_dir($this->plugin_dir . 'dist')) {
            error_log('MMC RELATIONSHIP_COUNSELLING Plugin: dist directory not found during activation.');
        }
    ...
    public function register_assets() {
        ...
        if (!file_exists($plugin_dir . 'dist') || !is_dir($plugin_dir . 'dist')) {
            error_log('MMC RELATIONSHIP_COUNSELLING Plugin: dist directory not found.');
            return;
        }
    ...
    private function register_from_manifest($manifest_file) {
        ...
        if (!$manifest || !is_array($manifest)) {
            error_log('MMC RELATIONSHIP_COUNSELLING Plugin: Invalid manifest file');
            return;
        }
        ...
        foreach ($manifest as $file => $details) {
            ...
            $handle = 'mmc-relationship_counselling-' . sanitize_title(basename($details['file']));
    ...
    private function register_direct() {
        ...
        foreach ($css_files as $index => $css_file) {
            ...
            $handle = 'mmc-relationship_counselling-css-' . $index;
        ...
        foreach ($js_files as $index => $js_file) {
            ...
            $handle = 'mmc-relationship_counselling-js-' . $index;
    ...
    public function maybe_enqueue_assets() {
        ...
        if (is_a($post, 'WP_Post') && has_shortcode($post->post_content, 'mmc_relationship_counselling')) {
            // Enqueue all registered styles
            $styles = wp_styles();
            foreach ($styles->registered as $handle => $style) {
                if (strpos($handle, 'mmc-relationship_counselling') === 0) {
    ...
    private function get_localized_data() {
        return apply_filters('mmc_relationship_counselling_localized_data', array(
            'ajaxUrl' => admin_url('admin-ajax.php'),
            'nonce' => wp_create_nonce('mmc_relationship_counselling_nonce'),
            'homeUrl' => home_url(),
            'siteUrl' => site_url(),
    ...
    public function render_react_app($atts) {
        ...
        $atts = shortcode_atts(array(
            'id' => 'mmc-relationship_counselling-' . uniqid(), // Generate a unique ID
            'class' => '',
            'category' => '', // Optional category filter
        ), $atts, 'mmc_relationship_counselling');
        ...
        return sprintf(
            ...
            '<noscript>' . esc_html__('JavaScript is required to view the RELATIONSHIP_COUNSELLING content.', 'mmc-relationship_counselling') . '</noscript>' .
            '<p class="react-loading-placeholder">%s</p>' .
            '</div>',
            esc_attr($atts['id']),
            esc_attr($atts['class']),
            $data_attrs,
            esc_html__('Loading RELATIONSHIP_COUNSELLING content...', 'mmc-relationship_counselling')
        );
    }
    ...
    public function add_debug_script() {
        ...
        <script>
        (function() {
            console.group('MMC RELATIONSHIP_COUNSELLING Debug Information');
                ...
                setTimeout(function() {
                    ...
                    if (placeholders.length > 0) {
                        ...
                        const reactScripts = Array.from(scripts).filter(script => 
                            script.src && (script.src.includes('mmc-relationship_counselling') || script.src.includes('dist/assets'))
                        );
 ...
 * @return MMC_RELATIONSHIP_COUNSELLING_App
 */
function mmc_relationship_counselling_plugin() {
    return MMC_RELATIONSHIP_COUNSELLING_App::get_instance();
}

// Initialize the plugin
mmc_relationship_counselling_plugin();