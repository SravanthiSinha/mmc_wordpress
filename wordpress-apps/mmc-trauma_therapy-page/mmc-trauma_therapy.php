<?php
/**
 * Plugin Name: MMC - Trauma Therapy PAGE
 * Description: Integration of MMC - Trauma Therapy PAGE with WordPress
 * Version: 1.0.0
 * Author: Sravanthi Sinha
 * Text Domain: mmc-trauma_therapy
 * License: GPL-2.0+
 ...
 * Main plugin class
 * 
 * Handles integration of  MMC - Trauma Therapy PAGE with WordPress
 */
class MMC_TRAUMA_THERAPY_App {
    /**
     * Plugin instance
     *
     * @var MMC_TRAUMA_THERAPY_App
     */
    ...
    private function __construct() {
        ...
        // Register shortcode
        add_shortcode('mmc_trauma_therapy', array($this, 'render_react_app'));
    ...
    public function activate() {
        ...
        if (version_compare(PHP_VERSION, '7.0', '<')) {
            ...
            wp_die(
                esc_html__('MMC TRAUMA_THERAPY requires PHP 7.0 or higher.', 'mmc-trauma_therapy'),
                esc_html__('Plugin Activation Error', 'mmc-trauma_therapy'),
        ...
        if (!file_exists($this->plugin_dir . 'dist') || !is_dir($this->plugin_dir . 'dist')) {
            error_log('MMC TRAUMA_THERAPY Plugin: dist directory not found during activation.');
        }
    ...
    public function register_assets() {
        ...
        if (!file_exists($plugin_dir . 'dist') || !is_dir($plugin_dir . 'dist')) {
            error_log('MMC TRAUMA_THERAPY Plugin: dist directory not found.');
            return;
        }
    ...
    private function register_from_manifest($manifest_file) {
        ...
        if (!$manifest || !is_array($manifest)) {
            error_log('MMC TRAUMA_THERAPY Plugin: Invalid manifest file');
            return;
        }
        ...
        foreach ($manifest as $file => $details) {
            ...
            $handle = 'mmc-trauma_therapy-' . sanitize_title(basename($details['file']));
    ...
    private function register_direct() {
        ...
        foreach ($css_files as $index => $css_file) {
            ...
            $handle = 'mmc-trauma_therapy-css-' . $index;
        ...
        foreach ($js_files as $index => $js_file) {
            ...
            $handle = 'mmc-trauma_therapy-js-' . $index;
    ...
    public function maybe_enqueue_assets() {
        ...
        if (is_a($post, 'WP_Post') && has_shortcode($post->post_content, 'mmc_trauma_therapy')) {
            // Enqueue all registered styles
            $styles = wp_styles();
            foreach ($styles->registered as $handle => $style) {
                if (strpos($handle, 'mmc-trauma_therapy') === 0) {
    ...
    private function get_localized_data() {
        return apply_filters('mmc_trauma_therapy_localized_data', array(
            'ajaxUrl' => admin_url('admin-ajax.php'),
            'nonce' => wp_create_nonce('mmc_trauma_therapy_nonce'),
            'homeUrl' => home_url(),
            'siteUrl' => site_url(),
    ...
    public function render_react_app($atts) {
        ...
        $atts = shortcode_atts(array(
            'id' => 'mmc-trauma_therapy-' . uniqid(), // Generate a unique ID
            'class' => '',
            'category' => '', // Optional category filter
        ), $atts, 'mmc_trauma_therapy');
        ...
        return sprintf(
            ...
            '<noscript>' . esc_html__('JavaScript is required to view the TRAUMA_THERAPY content.', 'mmc-trauma_therapy') . '</noscript>' .
            '<p class="react-loading-placeholder">%s</p>' .
            '</div>',
            esc_attr($atts['id']),
            esc_attr($atts['class']),
            $data_attrs,
            esc_html__('Loading TRAUMA_THERAPY content...', 'mmc-trauma_therapy')
        );
    }
    ...
    public function add_debug_script() {
        ...
        <script>
        (function() {
            console.group('MMC TRAUMA_THERAPY Debug Information');
                ...
                setTimeout(function() {
                    ...
                    if (placeholders.length > 0) {
                        ...
                        const reactScripts = Array.from(scripts).filter(script => 
                            script.src && (script.src.includes('mmc-trauma_therapy') || script.src.includes('dist/assets'))
                        );
 ...
 * @return MMC_TRAUMA_THERAPY_App
 */
function mmc_trauma_therapy_plugin() {
    return MMC_TRAUMA_THERAPY_App::get_instance();
}

// Initialize the plugin
mmc_trauma_therapy_plugin();