<?php
/**
 * Plugin Name: MMC - Family Cultural Dynamics PAGE
 * Description: Integration of MMC - Family Cultural Dynamics PAGE with WordPress
 * Version: 1.0.0
 * Author: Sravanthi Sinha
 * Text Domain: mmc-family_cultural_dynamics
 * License: GPL-2.0+
 ...
 * Main plugin class
 * 
 * Handles integration of  MMC - Family Cultural Dynamics PAGE with WordPress
 */
class MMC_Family_Cultural_Dynamics_App {
    /**
     * Plugin instance
     *
     * @var MMC_Family_Cultural_Dynamics_App
     */
     ...
     * Get plugin instance
     *
     * @return MMC_Family_Cultural_Dynamics_App
     */
    ...
    private function __construct() {
        ...
        // Register shortcode
        add_shortcode('mmc_family_cultural_dynamics', array($this, 'render_react_app'));
    ...
    public function activate() {
        ...
        if (version_compare(PHP_VERSION, '7.0', '<')) {
            ...
            wp_die(
                esc_html__('MMC Family_Cultural_Dynamics requires PHP 7.0 or higher.', 'mmc-family_cultural_dynamics'),
                esc_html__('Plugin Activation Error', 'mmc-family_cultural_dynamics'),
        ...
        if (!file_exists($this->plugin_dir . 'dist') || !is_dir($this->plugin_dir . 'dist')) {
            error_log('MMC Family_Cultural_Dynamics Plugin: dist directory not found during activation.');
        }
    ...
    public function register_assets() {
        ...
        if (!file_exists($plugin_dir . 'dist') || !is_dir($plugin_dir . 'dist')) {
            error_log('MMC Family_Cultural_Dynamics Plugin: dist directory not found.');
            return;
        }
    ...
    private function register_from_manifest($manifest_file) {
        ...
        if (!$manifest || !is_array($manifest)) {
            error_log('MMC Family_Cultural_Dynamics Plugin: Invalid manifest file');
            return;
        }
        ...
        foreach ($manifest as $file => $details) {
            ...
            $handle = 'mmc-family_cultural_dynamics-' . sanitize_title(basename($details['file']));
        ...
        if (isset($main_handle) && isset($main_file_url) && file_exists($main_file_path)) {
            ...
            wp_localize_script(
                $main_handle,
                'mmcChinesFamilyCulturalDynamicsData',
    ...
    private function register_direct() {
        ...
        foreach ($css_files as $index => $css_file) {
            ...
            $handle = 'mmc-family_cultural_dynamics-css-' . $index;
        ...
        foreach ($js_files as $index => $js_file) {
            ...
            $handle = 'mmc-family_cultural_dynamics-js-' . $index;
    ...
    public function maybe_enqueue_assets() {
        ...
        if (is_a($post, 'WP_Post') && has_shortcode($post->post_content, 'mmc_family_cultural_dynamics')) {
            // Enqueue all registered styles
            $styles = wp_styles();
            foreach ($styles->registered as $handle => $style) {
                if (strpos($handle, 'mmc-family_cultural_dynamics') === 0) {
    ...
    private function get_localized_data() {
        return apply_filters('mmc_family_cultural_dynamics_localized_data', array(
            'ajaxUrl' => admin_url('admin-ajax.php'),
            'nonce' => wp_create_nonce('mmc_family_cultural_dynamics_nonce'),
            'homeUrl' => home_url(),
            'siteUrl' => site_url(),
    ...
    public function render_react_app($atts) {
        ...
        $atts = shortcode_atts(array(
            'id' => 'mmc-family_cultural_dynamics-' . uniqid(), // Generate a unique ID
            'class' => '',
            'category' => '', // Optional category filter
        ), $atts, 'mmc_family_cultural_dynamics');
        ...
        return sprintf(
            ...
            '<noscript>' . esc_html__('JavaScript is required to view the Family_Cultural_Dynamics content.', 'mmc-family_cultural_dynamics') . '</noscript>' .
            '<p class="react-loading-placeholder">%s</p>' .
            '</div>',
            esc_attr($atts['id']),
            esc_attr($atts['class']),
            $data_attrs,
            esc_html__('Loading Family_Cultural_Dynamics content...', 'mmc-family_cultural_dynamics')
        );
    }
    ...
    public function add_debug_script() {
        ...
        <script>
        (function() {
            console.group('MMC Family_Cultural_Dynamics Debug Information');
                ...
                setTimeout(function() {
                    ...
                    if (placeholders.length > 0) {
                        ...
                        const reactScripts = Array.from(scripts).filter(script => 
                            script.src && (script.src.includes('mmc-family_cultural_dynamics') || script.src.includes('dist/assets'))
                        );
 ...
 * @return MMC_Family_Cultural_Dynamics_App
 */
function mmc_family_cultural_dynamics_plugin() {
    return MMC_Family_Cultural_Dynamics_App::get_instance();
}

// Initialize the plugin
mmc_family_cultural_dynamics_plugin();