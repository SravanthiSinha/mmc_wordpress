<?php
/**
 * Plugin Name: MMC - PRIVACY_POLICY
 * Description: Integration of PRIVACY_POLICY page with WordPress
 * Version: 1.0.0
 * Author: Sravanthi Sinha
 * Text Domain: mmc-privacy_policy
 * License: GPL-2.0+
 ...
 * Main plugin class
 * 
 * Handles integration of PRIVACY_POLICY page with WordPress
 */
class MMC_PRIVACY_POLICY_App {
    /**
     * Plugin instance
     *
     * @var MMC_PRIVACY_POLICY_App
     */
    ...
    private function __construct() {
        ...
        // Register shortcode
        add_shortcode('mmc_privacy_policy', array($this, 'render_react_app'));
    ...
    public function activate() {
        ...
        if (version_compare(PHP_VERSION, '7.0', '<')) {
            ...
            wp_die(
                esc_html__('MMC PRIVACY_POLICY requires PHP 7.0 or higher.', 'mmc-privacy_policy'),
                esc_html__('Plugin Activation Error', 'mmc-privacy_policy'),
        ...
        if (!file_exists($this->plugin_dir . 'dist') || !is_dir($this->plugin_dir . 'dist')) {
            error_log('MMC PRIVACY_POLICY Plugin: dist directory not found during activation.');
        }
    ...
    public function register_assets() {
        ...
        if (!file_exists($plugin_dir . 'dist') || !is_dir($plugin_dir . 'dist')) {
            error_log('MMC PRIVACY_POLICY Plugin: dist directory not found.');
            return;
        }
    ...
    private function register_from_manifest($manifest_file) {
        ...
        if (!$manifest || !is_array($manifest)) {
            error_log('MMC PRIVACY_POLICY Plugin: Invalid manifest file');
            return;
        }
        ...
        foreach ($manifest as $file => $details) {
            ...
            $handle = 'mmc-privacy_policy-' . sanitize_title(basename($details['file']));
    ...
    private function register_direct() {
        ...
        foreach ($css_files as $index => $css_file) {
            ...
            $handle = 'mmc-privacy_policy-css-' . $index;
        ...
        foreach ($js_files as $index => $js_file) {
            ...
            $handle = 'mmc-privacy_policy-js-' . $index;
    ...
    public function maybe_enqueue_assets() {
        ...
        if (is_a($post, 'WP_Post') && has_shortcode($post->post_content, 'mmc_privacy_policy')) {
            // Enqueue all registered styles
            $styles = wp_styles();
            foreach ($styles->registered as $handle => $style) {
                if (strpos($handle, 'mmc-privacy_policy') === 0) {
    ...
    private function get_localized_data() {
        return apply_filters('mmc_privacy_policy_localized_data', array(
            'ajaxUrl' => admin_url('admin-ajax.php'),
            'nonce' => wp_create_nonce('mmc_privacy_policy_nonce'),
            'homeUrl' => home_url(),
            'siteUrl' => site_url(),
    ...
    public function render_react_app($atts) {
        ...
        $atts = shortcode_atts(array(
            'id' => 'mmc-privacy_policy-' . uniqid(), // Generate a unique ID
            'class' => '',
            'category' => '', // Optional category filter
        ), $atts, 'mmc_privacy_policy');
        ...
        return sprintf(
            ...
            '<noscript>' . esc_html__('JavaScript is required to view the PRIVACY_POLICY content.', 'mmc-privacy_policy') . '</noscript>' .
            '<p class="react-loading-placeholder">%s</p>' .
            '</div>',
            esc_attr($atts['id']),
            esc_attr($atts['class']),
            $data_attrs,
            esc_html__('Loading PRIVACY_POLICY content...', 'mmc-privacy_policy')
        );
    }
    ...
    public function add_debug_script() {
        ...
        <script>
        (function() {
            console.group('MMC PRIVACY_POLICY Debug Information');
                ...
                setTimeout(function() {
                    ...
                    if (placeholders.length > 0) {
                        ...
                        const reactScripts = Array.from(scripts).filter(script => 
                            script.src && (script.src.includes('mmc-privacy_policy') || script.src.includes('dist/assets'))
                        );
 ...
 * @return MMC_PRIVACY_POLICY_App
 */
function mmc_privacy_policy_plugin() {
    return MMC_PRIVACY_POLICY_App::get_instance();
}

// Initialize the plugin
mmc_privacy_policy_plugin();