<?php
/**
 * Plugin Name: MMC - Contact PAGE
 * Description: Integration of MMC - Contact PAGE with WordPress
 * Version: 1.0.0
 * Author: Sravanthi Sinha
 * Text Domain: mmc-contact
 * License: GPL-2.0+
 ...
 * Main plugin class
 * 
 * Handles integration of  MMC - Contact PAGE with WordPress
 */
class MMC_CONTACT_App {
    /**
     * Plugin instance
     *
     * @var MMC_CONTACT_App
    ...
    private function __construct() {
        ...
        // Register shortcode
        add_shortcode('mmc_contact', array($this, 'render_react_app'));
    ...
    public function activate() {
        ...
        if (version_compare(PHP_VERSION, '7.0', '<')) {
            ...
            wp_die(
                esc_html__('MMC CONTACT requires PHP 7.0 or higher.', 'mmc-contact'),
                esc_html__('Plugin Activation Error', 'mmc-contact'),
        ...
        if (!file_exists($this->plugin_dir . 'dist') || !is_dir($this->plugin_dir . 'dist')) {
            error_log('MMC CONTACT Plugin: dist directory not found during activation.');
        }
    ...
    public function register_assets() {
        ...
        if (!file_exists($plugin_dir . 'dist') || !is_dir($plugin_dir . 'dist')) {
            error_log('MMC CONTACT Plugin: dist directory not found.');
            return;
        }
    ...
    private function register_from_manifest($manifest_file) {
        ...
        if (!$manifest || !is_array($manifest)) {
            error_log('MMC CONTACT Plugin: Invalid manifest file');
            return;
        }
        ...
        foreach ($manifest as $file => $details) {
            ...
            $handle = 'mmc-contact-' . sanitize_title(basename($details['file']));
    ...
    public function maybe_enqueue_assets() {
        ...
        if (is_a($post, 'WP_Post') && has_shortcode($post->post_content, 'mmc_contact')) {
            // Enqueue all registered styles
            $styles = wp_styles();
            foreach ($styles->registered as $handle => $style) {
                if (strpos($handle, 'mmc-contact') === 0) {
    ...
    private function get_localized_data() {
        return apply_filters('mmc_contact_localized_data', array(
            'ajaxUrl' => admin_url('admin-ajax.php'),
            'nonce' => wp_create_nonce('mmc_contact_nonce'),
            'homeUrl' => home_url(),
            'siteUrl' => site_url(),
    ...
    public function render_react_app($atts) {
        ...
        $atts = shortcode_atts(array(
            'id' => 'mmc-contact-' . uniqid(), // Generate a unique ID
            'class' => '',
        ...
        return sprintf(
            ...
            '<noscript>' . esc_html__('JavaScript is required to view the CONTACT content.', 'mmc-contact') . '</noscript>' .
            '<p class="react-loading-placeholder">%s</p>' .
            '</div>',
            esc_attr($atts['id']),
            esc_attr($atts['class']),
            $data_attrs,
            esc_html__('Loading CONTACT content...', 'mmc-contact')
        );
    }
    ...
    public function add_debug_script() {
        ...
        <script>
        (function() {
            console.group('MMC CONTACT Debug Information');
                ...
                setTimeout(function() {
                    ...
                    if (placeholders.length > 0) {
                        ...
                        const reactScripts = Array.from(scripts).filter(script => 
                            script.src && (script.src.includes('mmc-contact') || script.src.includes('dist/assets'))
                        );
 ...
 * @return MMC_CONTACT_App
 */
function mmc_contact_plugin() {
    return MMC_CONTACT_App::get_instance();
}
