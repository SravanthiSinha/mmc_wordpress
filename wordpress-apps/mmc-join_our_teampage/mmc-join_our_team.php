<?php
/**
 * Plugin Name: MMC - Join Our Team PAGE
 * Description: Integration of MMC - Join Our Team PAGE with WordPress
 * Version: 1.0.0
 * Author: Sravanthi Sinha
 * Text Domain: mmc-join_our_team
 * License: GPL-2.0+
 ...
 * Main plugin class
 * 
 * Handles integration of  MMC - Join Our Team PAGE with WordPress
 */
class MMC_JOIN_OUR_TEAM_App {
    /**
     * Plugin instance
     *
     * @var MMC_JOIN_OUR_TEAM_App
     */
    ...
    private function __construct() {
        ...
        // Register shortcode
        add_shortcode('mmc_join_our_team', array($this, 'render_react_app'));
    ...
    public function activate() {
        ...
        if (version_compare(PHP_VERSION, '7.0', '<')) {
            ...
            wp_die(
                esc_html__('MMC JOIN_OUR_TEAM requires PHP 7.0 or higher.', 'mmc-join_our_team'),
                esc_html__('Plugin Activation Error', 'mmc-join_our_team'),
        ...
        if (!file_exists($this->plugin_dir . 'dist') || !is_dir($this->plugin_dir . 'dist')) {
            error_log('MMC JOIN_OUR_TEAM Plugin: dist directory not found during activation.');
        }
    ...
    public function register_assets() {
        ...
        if (!file_exists($plugin_dir . 'dist') || !is_dir($plugin_dir . 'dist')) {
            error_log('MMC JOIN_OUR_TEAM Plugin: dist directory not found.');
            return;
        }
    ...
    private function register_from_manifest($manifest_file) {
        ...
        if (!$manifest || !is_array($manifest)) {
            error_log('MMC JOIN_OUR_TEAM Plugin: Invalid manifest file');
            return;
        }
        ...
        foreach ($manifest as $file => $details) {
            ...
            $handle = 'mmc-join_our_team-' . sanitize_title(basename($details['file']));
    ...
    private function register_direct() {
        ...
        foreach ($css_files as $index => $css_file) {
            ...
            $handle = 'mmc-join_our_team-css-' . $index;
        ...
        foreach ($js_files as $index => $js_file) {
            ...
            $handle = 'mmc-join_our_team-js-' . $index;
    ...
    public function maybe_enqueue_assets() {
        ...
        if (is_a($post, 'WP_Post') && has_shortcode($post->post_content, 'mmc_join_our_team')) {
            // Enqueue all registered styles
            $styles = wp_styles();
            foreach ($styles->registered as $handle => $style) {
                if (strpos($handle, 'mmc-join_our_team') === 0) {
    ...
    private function get_localized_data() {
        return apply_filters('mmc_join_our_team_localized_data', array(
            'ajaxUrl' => admin_url('admin-ajax.php'),
            'nonce' => wp_create_nonce('mmc_join_our_team_nonce'),
            'homeUrl' => home_url(),
            'siteUrl' => site_url(),
    ...
    public function render_react_app($atts) {
        ...
        $atts = shortcode_atts(array(
            'id' => 'mmc-join_our_team-' . uniqid(), // Generate a unique ID
            'class' => '',
            'category' => '', // Optional category filter
        ), $atts, 'mmc_join_our_team');
        ...
        return sprintf(
            ...
            '<noscript>' . esc_html__('JavaScript is required to view the JOIN_OUR_TEAM content.', 'mmc-join_our_team') . '</noscript>' .
            '<p class="react-loading-placeholder">%s</p>' .
            '</div>',
            esc_attr($atts['id']),
            esc_attr($atts['class']),
            $data_attrs,
            esc_html__('Loading JOIN_OUR_TEAM content...', 'mmc-join_our_team')
        );
    }
    ...
    public function add_debug_script() {
        ...
        <script>
        (function() {
            console.group('MMC JOIN_OUR_TEAM Debug Information');
                ...
                setTimeout(function() {
                    ...
                    if (placeholders.length > 0) {
                        ...
                        const reactScripts = Array.from(scripts).filter(script => 
                            script.src && (script.src.includes('mmc-join_our_team') || script.src.includes('dist/assets'))
                        );
 ...
 * @return MMC_JOIN_OUR_TEAM_App
 */
function mmc_join_our_team_plugin() {
    return MMC_JOIN_OUR_TEAM_App::get_instance();
}

// Initialize the plugin
mmc_join_our_team_plugin();