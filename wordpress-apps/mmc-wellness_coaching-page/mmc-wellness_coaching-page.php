<?php
/**
 * Plugin Name: MMC - WELLNESS_COACHING
 * Description: Integration of React Vite app with WordPress
 * Version: 1.0.0
 * Author: Sravanthi Sinha
 * Text Domain: mmc-wellness_coaching
 * License: GPL-2.0+
 ...
 */
class MMC_WELLNESS_COACHING_App {
    /**
     * Plugin instance
     *
     * @var MMC_WELLNESS_COACHING_App
     */
     ...
     * Get plugin instance
     *
     * @return MMC_WELLNESS_COACHING_App
     */
    ...
    private function __construct() {
        ...
        // Register shortcode
        add_shortcode('mmc_wellness_coaching', array($this, 'render_react_app'));
    ...
    public function activate() {
        ...
        if (version_compare(PHP_VERSION, '7.0', '<')) {
            ...
            wp_die(
                esc_html__('MMC WELLNESS_COACHING requires PHP 7.0 or higher.', 'mmc-wellness_coaching'),
                esc_html__('Plugin Activation Error', 'mmc-wellness_coaching'),
        ...
        if (!file_exists($this->plugin_dir . 'dist') || !is_dir($this->plugin_dir . 'dist')) {
            error_log('MMC WELLNESS_COACHING Plugin: dist directory not found during activation.');
        }
    ...
    public function register_assets() {
        ...
        if (!file_exists($plugin_dir . 'dist') || !is_dir($plugin_dir . 'dist')) {
            error_log('MMC WELLNESS_COACHING Plugin: dist directory not found.');
            return;
        }
    ...
    private function register_from_manifest($manifest_file) {
        ...
        if (!$manifest || !is_array($manifest)) {
            error_log('MMC WELLNESS_COACHING Plugin: Invalid manifest file');
            return;
        }
        ...
        foreach ($manifest as $file => $details) {
            ...
            $handle = 'mmc-wellness_coaching-' . sanitize_title(basename($details['file']));
    ...
    private function register_direct() {
        ...
        foreach ($css_files as $index => $css_file) {
            ...
            $handle = 'mmc-wellness_coaching-css-' . $index;
        ...
        foreach ($js_files as $index => $js_file) {
            ...
            $handle = 'mmc-wellness_coaching-js-' . $index;
    ...
    public function maybe_enqueue_assets() {
        ...
        if (is_a($post, 'WP_Post') && has_shortcode($post->post_content, 'mmc_wellness_coaching')) {
            // Enqueue all registered styles
            $styles = wp_styles();
            foreach ($styles->registered as $handle => $style) {
                if (strpos($handle, 'mmc-wellness_coaching') === 0) {
    ...
    private function get_localized_data() {
        return apply_filters('mmc_wellness_coaching_localized_data', array(
            'ajaxUrl' => admin_url('admin-ajax.php'),
            'nonce' => wp_create_nonce('mmc_wellness_coaching_nonce'),
            'homeUrl' => home_url(),
            'siteUrl' => site_url(),
    ...
    public function render_react_app($atts) {
        ...
        $atts = shortcode_atts(array(
            'id' => 'mmc-wellness_coaching-' . uniqid(), // Generate a unique ID
            'class' => '',
            'category' => '', // Optional category filter
        ), $atts, 'mmc_wellness_coaching');
        ...
        return sprintf(
            ...
            '<noscript>' . esc_html__('JavaScript is required to view the WELLNESS_COACHING content.', 'mmc-wellness_coaching') . '</noscript>' .
            '<p class="react-loading-placeholder">%s</p>' .
            '</div>',
            esc_attr($atts['id']),
            esc_attr($atts['class']),
            $data_attrs,
            esc_html__('Loading WELLNESS_COACHING content...', 'mmc-wellness_coaching')
        );
    }
    ...
    public function add_debug_script() {
        ...
        <script>
        (function() {
            console.group('MMC WELLNESS_COACHING Debug Information');
                ...
                setTimeout(function() {
                    ...
                    if (placeholders.length > 0) {
                        ...
                        const reactScripts = Array.from(scripts).filter(script => 
                            script.src && (script.src.includes('mmc-wellness_coaching') || script.src.includes('dist/assets'))
                        );
 ...
 * @return MMC_WELLNESS_COACHING_App
 */
function mmc_wellness_coaching_plugin() {
    return MMC_WELLNESS_COACHING_App::get_instance();
}

// Initialize the plugin
mmc_wellness_coaching_plugin();